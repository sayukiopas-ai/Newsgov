...
export default function EditBoardModal({ board, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await updateBoard(board._id, { title: title.trim(), description })
    onClose()
  }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl p-5 sm:p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg sm:text-xl font-bold text-gray-800">Edit Board</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm sm:text-base"
              autoFocus
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 resize-none text-sm sm:text-base"
              rows={3}
            />
          </div>
